...
{if(value=='')return value;var ts=this.thousandSeparator,cs=((cs=$.trim(this.centSeparator))!=='')?cs:'.',v=$.fbuilder.parseVal(value,ts,cs),parts=[],counter=0,str='',sign='';if(!isNaN(v))
...
{return false;}};if(!('dateddmmyyyy'in $.validator.methods))$.validator.addMethod("dateddmmyyyy",validator);if(!('datemmddyyyy'in $.validator.methods))$.validator.addMethod("datemmddyyyy",validator);me.setDefaultDate();me.setDefaultTime();me.setEvents();me.set_date_time();},val:function(raw)
{raw=raw||false;var e=$('[id="'+this.name+'"]:not(.ignore)');if(e.length)
{var v=e.val(),rt;if(raw)return $.fbuilder.parseValStr(v);if(this.dformat=='yyyy/mm/dd'||this.dformat=='yyyy/dd/mm')
...
{raw=raw||false;var v,me=this,e=$('[id="'+me.name+'"]:checked:not(.ignore)');me.merge=me.merge&&!raw;if(!me.merge)v=[];if(e.length)
...
{var _match,field_regexp=new RegExp('(fieldname\\d+'+suffix+')(\\|r)?([\\D\\b])','i'),__ME__=(__ME__)?$.fbuilder['forms'][suffix].getItem(__ME__).val():'';$.fbuilder['currentFormId']=$.fbuilder['forms'][suffix].formId;eq='('+eq+')';while(_match=field_regexp.exec(eq))
{var field=$.fbuilder['forms'][suffix].getItem(_match[1]),v='';if(field)
{v=field.val((_match[2])?true:false);if($.isNumeric(v))v='('+v+')';else if(typeof v=='object'&&typeof window.JSON!='undefined')v=JSON.stringify(v);}
eq=eq.replace(_match[0],v+''+_match[3]);}
try
