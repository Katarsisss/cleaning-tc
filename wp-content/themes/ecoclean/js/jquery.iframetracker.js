...
(function($){
  // Registering new tracking handler
	$.fn.iframeTracker = function(handler){
		...
		$(this)
			.on('mouseover', {handler: handler}, function(e){
				e.data.handler.over = true;
				try{ e.data.handler.overCallback(this); } catch(ex){}
			})
			.on('mouseout',  {handler: handler}, function(e){
	...
	$.iframeTracker = {
		...
		init: function(){
			...
			$(document).on('mousemove', function(e){ // Focus back to page
				if( document.activeElement.tagName == 'IFRAME' ){
					$.iframeTracker.focusRetriever.on('focus');
					$.iframeTracker.focusRetrieved = true;
				}
			});
			// Blur doesn't works correctly on IE8-, so we need to trigger it manually
			if( this.isIE8AndOlder ){
				this.focusRetriever.on('blur', function(e){
					e.stopPropagation();
					e.preventDefault();
					$.iframeTracker.windowLoseFocus(e);
				});
			...
			if( this.isIE8AndOlder ){
				$('body').on('click', function(e){ $(window).on('focus'); });
